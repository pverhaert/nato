...
@php
    $checked = filter_var($checked, FILTER_VALIDATE_BOOLEAN); //Deze zijn eigenlijk niet nodig bij standaard html boolean attributes, presence is enough to indicate true.
    $disabled = filter_var($disabled, FILTER_VALIDATE_BOOLEAN);
    //$checked = ($disabled === 'true' || $disabled === true); // Alternatieve schrijfwijze, persoonlijke voorkeur
...
<label
    ...
    <input type="checkbox" class="hidden peer"
           name="{{ $name }}"
           id="{{ $id }}"  {{-- $name is fout lijkt mij --}}
           value="{{ $value }}"
           {{ $checked ? 'checked' : '' }}
           {{ $disabled ? 'disabled' : '' }}
           {{ $attributes->get('x-model') ? 'x-model=' . $attributes->get('x-model'): ''}}
           {{ $attributes->get('wire:model.live') ? 'wire:model.live=' . $attributes->get('wire:model.live'): '' }}
